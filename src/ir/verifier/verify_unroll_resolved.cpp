...
/// ``SplitChunkedLoops`` — non-chunked Unroll loops are expanded into
/// SeqStmts by ``UnrollLoops``, and chunked Unroll loops are decomposed
/// into nested Sequential loops by ``SplitChunkedLoops``.
///
/// Any leftover indicates either a new code path that forgot to route
/// through UnrollLoops, or a chunked loop that SplitChunkedLoops failed
/// to demote.
  ...
  void VisitStmt_(const ForStmtPtr& op) override {
    if (op->kind_ == ForKind::Unroll) {
      diagnostics_.emplace_back(DiagnosticSeverity::Error, "UnrollResolved", 0,
                                "ForKind::Unroll survived past SplitChunkedLoops in function '" +
                                    func_name_ +
                                    "'. This kind is a compile-time marker — UnrollLoops expands "
                                    "non-chunked loops into SeqStmts, and SplitChunkedLoops demotes "
                                    "chunked Unroll loops to Sequential.",
                                op->span_);
    }
