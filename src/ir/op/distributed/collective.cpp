...
TypePtr DeduceTensorAllToAllVType(const std::vector<ExprPtr>& args,
  ...
  // already-general check). Counts publish on the recv_counts window (see
  // recv_counts comment below), never on signal.
  auto signal_type = As<DistributedTensorType>(args[2]->GetType());
...
TypePtr DeduceBuiltinTensorAllToAllVType(const std::vector<ExprPtr>& args,
  ...
  // mirroring the composite deducer's own relaxation above; counts publish on
  // recv_counts, never on signal.
  auto signal_type = As<DistributedTensorType>(args[2]->GetType());
