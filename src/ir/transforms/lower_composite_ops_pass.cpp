...
//
// Hand-rolled reference: tests/st/distributed/test_l3_ring_allreduce.py
...
ExprPtr LowerTensorRingAllReduceRule(const CallPtr& call, const std::vector<ExprPtr>& args,
  ...
  // chunk_size = SIZE // NR  (dynamic-NR-safe via MakeFloorDiv)
  auto size_expr = target_type->shape_[1];
  auto chunk_size = MakeFloorDiv(size_expr, comm.nranks_idx, span);
  ...
  b.EmitFor(
      ...
      [&](LoweringBuilder& body, const VarPtr& ag_step_var) {
        ...
        auto l1 = MakeSub(my_rank_idx, one_idx, span);
        auto l2 = MakeAdd(l1, comm.nranks_idx, span);
        auto left_peer = body.Bind("ag_left", MakeFloorMod(l2, comm.nranks_idx, span), span);
