...
void DistributedCodegen::EmitShapeDimVarDefs(const ir::FunctionPtr& func) {
  ...
  auto process_type = [&](const ir::TypePtr& type, const std::string& tensor_name) {
    auto shaped = ir::AsTensorTypeLike(type);
    if (!shaped) return;
  ...
  for (const auto& param : func->params_) {
    process_type(param->GetType(), "\"" + SanitizeName(param->name_hint_) + "\"");
  }
  ...
  for (const auto& var : ordered_vars) {
    ...
    auto it = var_source.find(var.get());
    if (it == var_source.end() || it->second.first.empty()) continue;
    emitter_.EmitLine(name + " = tensors[" + it->second.first + "].shape[" +
