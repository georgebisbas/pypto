...
- **端到端（ST）**：`tests/st/distributed/test_l3_allreduce.py`（mesh allreduce；
  ...
  ``--device=0,1,2,3`` 或 ``--device=0-3``）、
  `test_l3_ring_allreduce.py`（ring allreduce — 分块 reduce-scatter +
  allgather 调度，单块 InCore 核函数；golden 相同，不同算法：
  每步远程流量 O(N/P) vs mesh O(N)）、
