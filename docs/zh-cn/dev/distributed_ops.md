...
PyPTO 支持三种跨 rank 集合通信交付模式：

1. **手写 InCore** — 用户直接组合 `pld.system.notify` /
   `pld.system.wait` / `pld.tile.remote_load` / `tile.add` / `tile.store`。
   已有 mesh allreduce（P=2、P=4 动态 `NR`）、broadcast（P=2）、ring
   allreduce（RS+AG，P=2、P=4）、allgather 与 reduce_scatter 的参考 ST。

2. **复合 intrinsic** — `pld.tensor.allreduce(target, signal,
   op=ReduceOp.Sum)` 为 InCore 复合算子，在 `LowerCompositeOps`（pass 14）
   中降为 5 阶段分解
   ([#1746](https://github.com/hw-native-sys/pypto/pull/1746))。更多复合
   集合（`all_gather`、`reduce_scatter`、`broadcast`、`barrier`）在
   [#1782](https://github.com/hw-native-sys/pypto/pull/1782) 中推进。这是
   未来 InCore 集合的推荐交付方式。

3. **HOST 层 builtin** — host_orch 上的张量集合（如对 host tensor 的
   `pld.tensor.allreduce`）经 `LowerHostTensorCollectives`（pass 38）降为
   内部 builtin chip dispatch
   ([#1798](https://github.com/hw-native-sys/pypto/pull/1798))。适用于需
   跨 chip 编排调度、而非纯 InCore 配方的集合。

| 原语 | Mesh（手写） | Ring RS+AG | 复合 intrinsic | HOST builtin |
|------|-------------|------------|----------------|--------------|
| all_reduce | ✅ P=2、P=4 动态 `NR` | ✅ P=2、P=4 ([#1778](https://github.com/hw-native-sys/pypto/pull/1778)) | ✅ 仅 `ReduceOp.Sum` ([#1746](https://github.com/hw-native-sys/pypto/pull/1746)) | ✅ ([#1798](https://github.com/hw-native-sys/pypto/pull/1798)) |
| all_gather | ✅ `main` 上 ST ([#1575](https://github.com/hw-native-sys/pypto/pull/1575))；动态 `NR` 对等已关闭 ([#1726](https://github.com/hw-native-sys/pypto/pull/1726)) | — | 🟡 [#1782](https://github.com/hw-native-sys/pypto/pull/1782) pr_open | 📋 计划中 |
| reduce_scatter | ✅ `main` 上 ST ([#1575](https://github.com/hw-native-sys/pypto/pull/1575))；动态 `NR` 对等已关闭 ([#1726](https://github.com/hw-native-sys/pypto/pull/1726)) | — | 🟡 [#1782](https://github.com/hw-native-sys/pypto/pull/1782) pr_open | 📋 计划中 |
| broadcast | ✅ P=2 ([#1631](https://github.com/hw-native-sys/pypto/pull/1631)) | — | 🟡 [#1782](https://github.com/hw-native-sys/pypto/pull/1782) pr_open | 📋 计划中 |
| all_to_all(_v) | 🟡 仅 simpler 示例 | — | 📋 可能为 HOST 层 | 📋 计划中 |
| barrier | notify/wait 对（各 ST） | — | 🟡 [#1782](https://github.com/hw-native-sys/pypto/pull/1782) pr_open | — |
| new_group | — | — | 📋 需子组基础设施（plan 17） | — |

图例：✅ 已合并至 `main`；🟡 进行中的 PR；📋 尚未实现。

上游跟踪：[#1189](https://github.com/hw-native-sys/pypto/issues/1189)
（「Add orchestration-level collective communication operators」）。

### 多组语义

每次 `pld.alloc_window_buffer` 分配到不同设备子集时，会产生独立的通信域与
隔离窗口。多组在同一设备上重叠时，运行时为每组维护独立的 `device_ctx`。
`pl.new_group(ranks)` 尚未实现 —— 见 plan 17 子组语义加固。
