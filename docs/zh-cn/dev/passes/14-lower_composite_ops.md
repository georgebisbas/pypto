...
把组合 (composite) tile / distributed 算子降级 (lower) 为一组基本算子的组合，使代码生成 (codegen) 不再需要发射高层 (high-level) 指令。当前支持 `tile.sin` / `tile.cos`（FP32 Cody-Waite + Horner）以及 `pld.tensor.*` 分布式集合通信算子（`allreduce`、`allgather`、`reduce_scatter`、`broadcast`、`barrier`）。新的组合算子只需在 Pass 文件内部的分发表 (dispatch table) 里加一条降级规则，无需改动分发器本身。
...
本 Pass 同时降级 `pld.tensor.*` 系列的窗口绑定 (window-bound) 分布式集合通信算子。每个集合通信算子都是一个组合 `Call`，展开为 notify / wait / `pld.tile.remote_load` / `tile.store` 序列。这些规则共享同一套 signal buffer 约定：使用窗口绑定的 INT32 `signal` 矩阵作为跨卡屏障，且**每次调用都需要新分配的 buffer**。
...
- Phase 3：对每个 peer `p != rank`，`remote_load` 该 peer 的行并 `tile.store` 到本地 `target`

`target` 形状为 `[NR, SIZE]`；每个 rank 在调用前把自己的 chunk 放在 `[my_rank, 0:SIZE]`。降级后每个 rank 的本地副本都持有完整的 `[NR, SIZE]` gathered 张量。不需要 post-gather 屏障，因为规则只写入调用者自己的窗口，后续读取在调用之后程序序发生。
...
- Phase 2a：notify-all（`Set 1`）
- Phase 2b：wait-all（`Ge 1`）
- Phase 3：每个非 root rank 通过 `remote_load` 读取 root rank 的切片并 `tile.store` 到本地；root rank 原地拷贝自己的切片
...
所有分布式规则在 wait 谓词上都使用 `kGe` 而非 `kEq`。单次调用内 cell 单调递增，但慢的 rank 第一次轮询时，如果快的 peer 已经完成 Phase 3 的 remote_load 并开始下一轮 notify，cell 可能已经超过阈值。此时 `kEq` 会死锁，`kGe` 不会。可自重置的写法（调用结束时 set-to-zero / `Eq 0`）受 PTOAS issue #797 阻塞，后续运行时修复落地后才能切换。
