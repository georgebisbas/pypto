# The Distributed Ladder

The `pld` vocabulary taught step by step: fifteen runnable programs, one
concept each, from "hello rank" to a kernel that composes three collectives.
...
The chapter reference tells you what `pld` *is*; this ladder shows you what it
*does*. Each step is a small, golden-validated program that teaches exactly one
abstraction, and the steps are ordered so you build each idea from the
primitives before a builtin replaces it:
...
> **Reveal discipline:** the builtins (`pld.tensor.barrier`,
> `pld.tensor.allreduce`, …) are never mentioned before the step that reveals
> them. By the time one appears, you have already written the hand-rolled
> version and know what it lowers to.
...
| 04 | `04_barrier.py` | Signals only: `notify(AtomicAdd)`/`wait(Ge)`; reusable N-rank barrier; reveal `pld.tensor.barrier` | ✅ shipped |
...
it, and the ladder step that teaches it. The **coverage contract** for the
ladder: nothing exists in code without being teachable from an example.

### System substrate

| Abstraction | Purpose | Chapter section | Ladder step |
| ----------- | ------- | --------------- | ----------- |
| `pld.world_size()` | Number of ranks in the world | [02-primitives](02-primitives.md) §System Substrate | 01 |
| `pld.get_comm_ctx(dt)` | Resolve the comm context a `DistributedTensor` belongs to | [02-primitives](02-primitives.md) §System Substrate | 04 |
| `pld.rank(ctx)` | This rank's index in the context | [02-primitives](02-primitives.md) §System Substrate | 04 |
| `pld.nranks(ctx)` | Rank count in the context | [02-primitives](02-primitives.md) §System Substrate | 04 |
...
| Abstraction | Purpose | Chapter section | Ladder step |
| ----------- | ------- | --------------- | ----------- |
| `pld.system.notify(...)` | Increment a signal cell on a peer | [02-primitives](02-primitives.md) §Notify & Wait | 04 |
| `pld.system.wait(...)` | Block until a signal cell reaches a threshold | [02-primitives](02-primitives.md) §Notify & Wait | 04 |
| `pld.NotifyOp.AtomicAdd` | Notify mode that adds (keeps barriers reusable) | [02-primitives](02-primitives.md) §Choosing NotifyOp and WaitCmp | 04 |
