...
- `signal` — DistributedTensor INT32 `[NR, S]`（自清理信用屏障；可在多次调用间复用）。
  `S >= 2`：最后一列（`S - 1`）是全局 count-ready 槽位（HOST/CHIP kernel 中
  权威的 `recv_counts` 握手，仅由 `block_idx == 0` 负责）；列 `[0, S - 1)` 是
  块感知屏障中每个 block 私有的信号槽位（RFC #2521 K2）——一个 block 在
  `peer * S + block_idx` 处寻址自己的屏障槽位，因此 `S - 1` 必须不小于某个
  `core_num` 实际产生的已准入 block 数 `B`（在 HOST 入口处检查，见下文）
