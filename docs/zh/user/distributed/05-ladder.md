# 分布式阶梯（The Distributed Ladder）

`pld` 词汇表按步骤讲解：十五个可运行的程序，每步一个概念，从 "hello rank"
到组合三种集合通信的 kernel。
...
参考章节告诉你 `pld` *是什么*；这个阶梯展示它 *做什么*。每一步都是一个
小型、经 golden 校验的程序，只教授一个抽象，并且顺序经过设计：先用原语
手工构建每个概念，再让内置原语取代它：
...
> **揭示纪律（Reveal discipline）：** 在揭示它们的步骤之前，绝不提前提及
> 内置原语（`pld.tensor.barrier`、`pld.tensor.allreduce` 等）。等到它们出现时，
> 你已经写出手工版本，并知道它们 lower 成什么。
...
| 04 | `04_barrier.py` | 仅信号：`notify(AtomicAdd)`/`wait(Ge)`；可复用 N-rank barrier；揭示 `pld.tensor.barrier` | ✅ 已交付 |
...
每个 `pld` 抽象：一行用途、文档它的章节小节、教授它的阶梯步骤。
阶梯的**覆盖契约**：代码中存在的任何内容都必须能由某个示例教授。

### 系统基座（System substrate）

| 抽象 | 用途 | 章节小节 | 阶梯步骤 |
| ---- | ---- | -------- | -------- |
| `pld.world_size()` | world 中的 rank 数量 | [02-primitives](02-primitives.md) §系统基座 | 01 |
| `pld.get_comm_ctx(dt)` | 解析 `DistributedTensor` 所属的通信上下文 | [02-primitives](02-primitives.md) §系统基座 | 04 |
| `pld.rank(ctx)` | 本 rank 在上下文中的索引 | [02-primitives](02-primitives.md) §系统基座 | 04 |
| `pld.nranks(ctx)` | 上下文中的 rank 数量 | [02-primitives](02-primitives.md) §系统基座 | 04 |
...
| 抽象 | 用途 | 章节小节 | 阶梯步骤 |
| ---- | ---- | -------- | -------- |
| `pld.system.notify(...)` | 在对端增加一个信号单元 | [02-primitives](02-primitives.md) §Notify 与 Wait | 04 |
| `pld.system.wait(...)` | 阻塞直到信号单元达到阈值 | [02-primitives](02-primitives.md) §Notify 与 Wait | 04 |
| `pld.NotifyOp.AtomicAdd` | 累加的 notify 模式（使 barrier 可复用） | [02-primitives](02-primitives.md) §选择 NotifyOp 与 WaitCmp | 04 |
