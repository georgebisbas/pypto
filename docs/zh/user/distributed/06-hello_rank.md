...
- **标量活在 AICPU 上。** `rank` 以 `INT32` 标量到达。kernel 将其转换为
  `FP32`，并折入*向量*运算（`x*rank + x`）。若写成 `rank_f32 + 1.0` 这样的
...
- `x` 和 `y` 携带 **world 形状** `[N_RANKS, 1, SIZE]`——rank `r` 的 slice 是
  `x[r]` / `y[r]`。
...
| `arith.addf explicitly marked illegal` | AI 核上的标量 `FP32` 算术 | 将常量折入向量运算（`x*rank + x`） |
