...
pass or codegen path: the actual runtime call site — the self-contained
``all_to_all_v/templates/entry.cpp.in`` template (RFC #2521 K2) — is compiled
separately from these compiler sources, so it carries its own C++ mirror of this
exact formula rather than linking against it. This function exists to pin the
formula down and test it once, in a form that is directly testable today.
"""
...
def cal_all_to_all_v_blocks(p: int, l: int) -> int:  # noqa: E741 - RFC's own symbol names
    ...
    Raises:
        TypeError: If ``p`` or ``l`` is not exactly ``int``. ``bool`` and floats are
            rejected so the annotated ``int`` return contract holds, and ``int``
            subclasses (``IntEnum`` members, NumPy integer scalars) are rejected
            too — an exact-type check keeps foreign integer types out of the shape
            and launch-width arithmetic.
