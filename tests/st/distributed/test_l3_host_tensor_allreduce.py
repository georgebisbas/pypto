...
def _build_host_allreduce_loop(rounds: int = 3):
    ...
    SynthesizeAllReduceSignals hoists ONE shared mesh signal and rewrites the
    in-loop call to use it (previously rejected as a single-use signal inside a
    repeating scope). Loop-carried reuse is safe because the builtin kernels
    self-clear their barrier cells after each call (self-clearing epilogue).
    """
    ...
    class HostTensorAllReduceLoop:
        ...
        def host_orch(
            ...
            # is synthesized for the whole function and reused across iterations.
