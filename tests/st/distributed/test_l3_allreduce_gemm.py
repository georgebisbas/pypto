...
class TestL3AllReduceGemm:
    ...
    def test_allreduce_gemm_determinism(self, test_config, device_ids):
        """Ten consecutive runs must match the torch golden (fixed peer order)."""
        if len(device_ids) < 2:
            pytest.skip(f"allreduce-gemm needs 2 devices, got {device_ids}")
        ...
        for _ in range(10):
            ...
            compiled(a, b, partials, outputs)
            assert torch.allclose(outputs, expected, rtol=1e-5, atol=1e-5), (
                "allreduce-gemm determinism failed"
            )
