...
"""L3 distributed ST: host-orchestrator ``pld.tensor.all_to_all`` dispatch.

The 4-arg InCore composite (pld.tensor.all_to_all(inp, data, signal, out))
is the only supported path — it routes through per-device private tensors.
The 2-arg HOST builtin form (pld.tensor.all_to_all(data, signal)) is NOT
supported because the HOST builtin infrastructure uses a shared HCCL window
where CommRemotePtr cannot distinguish between peer devices (windowsIn[0]
== windowsIn[1]), making personalized all_to_all exchange impossible.
Allreduce/broadcast/allgather work with the 2-arg form because they produce
