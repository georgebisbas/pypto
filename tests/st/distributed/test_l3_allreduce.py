...
ST coverage: **P=2** (default CI / 2-device hosts) and **P=4** (4-device
hosts via ``--device=0-3``). Both use the same N-rank program body.
"""
...
class TestL3AllReduce:
    ...
    def test_allreduce_p2(self, test_config, device_ids):
        if len(device_ids) < 2:
            pytest.skip(f"allreduce P=2 needs 2 devices, got {device_ids}")
        ...
        assert torch.allclose(outputs, expected), (
            f"allreduce P=2 mismatch: max diff = {(outputs - expected).abs().max().item()}"
        )

    def test_allreduce_p4(self, test_config, device_ids):
        if len(device_ids) < 4:
            pytest.skip(f"allreduce P=4 needs 4 devices, got {device_ids}")
        ...
        expected = _expected_allreduce(inputs)
        assert torch.allclose(outputs, expected), (
            f"allreduce P=4 mismatch: max diff = {(outputs - expected).abs().max().item()}"
        )
