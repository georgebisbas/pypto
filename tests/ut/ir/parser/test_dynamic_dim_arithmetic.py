...
def test_dynamic_dim_mul_in_shape():
    ...
    # Shape[0] should be ir.Mul wrapping Var("NR") and ConstInt(64).
    dim0 = return_type.shape[0]
    assert isinstance(dim0, ir.Mul), f"expected ir.Mul, got {type(dim0).__name__}"
...
def test_dynamic_dim_add_in_shape():
    ...
    assert isinstance(return_type, ir.TensorType)
    dim0 = return_type.shape[0]
    assert isinstance(dim0, ir.Add), f"expected ir.Add, got {type(dim0).__name__}"
...
def test_dynamic_dim_sub_in_shape():
    ...
    assert isinstance(return_type, ir.TensorType)
    dim0 = return_type.shape[0]
    assert isinstance(dim0, ir.Sub), f"expected ir.Sub, got {type(dim0).__name__}"
...
def test_dynamic_dim_floordiv_in_shape():
    ...
    assert isinstance(return_type, ir.TensorType)
    dim0 = return_type.shape[0]
    assert isinstance(dim0, ir.FloorDiv), f"expected ir.FloorDiv, got {type(dim0).__name__}"
...
def test_dynamic_dim_mod_in_shape():
    ...
    assert isinstance(return_type, ir.TensorType)
    dim0 = return_type.shape[0]
    assert isinstance(dim0, ir.FloorMod), f"expected ir.FloorMod, got {type(dim0).__name__}"
