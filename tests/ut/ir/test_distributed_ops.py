...
def test_all_to_all_v_accepts_wider_signal_stride(signal_stride):
    ...
    K2's block-aware barrier addresses a private per-block lane at
    signal_stride S; counts publish on recv_counts, never on signal, so no
    column is reserved. core_num is still gated at 1 elsewhere, so this only
    proves the shape check itself was relaxed, not that block_num>1 is
